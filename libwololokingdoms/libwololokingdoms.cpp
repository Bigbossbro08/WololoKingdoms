...
public:
  ...
  wkconvert_handle(WKConverter* converter, Listener* listener)
...
extern "C" void wksettings_resource_path(wksettings_t inst,
                                         const path_char_t* path) {
...
extern "C" void wksettings_output_path(wksettings_t inst,
                                       const path_char_t* path) {
  inst->outPath = path;
}

extern "C" void wksettings_voobly_path(wksettings_t inst,
                                       const path_char_t* path) {
  inst->vooblyDir = path;
}
...
extern "C" void wksettings_data_mod(wksettings_t inst, const char* name,
                                    const char* exe, const char* version,
                                    int flags, const char* exe2) {
  // unimplemented
  /* inst->dataModList.emplace_back(name, exe, version, flags, exe2 == nullptr ?
   * "" : exe2); */
}
...
extern "C" void wkconverter_on_finished(wkconverter_t inst,
                                        void (*onFinished)(void*)) {
  inst->listener->onFinished = onFinished;
}

extern "C" void wkconverter_on_log(wkconverter_t inst,
                                   void (*onLog)(void*, const char*)) {
  inst->listener->onLog = onLog;
}

extern "C" void wkconverter_on_set_info(wkconverter_t inst,
                                        void (*onSetInfo)(void*, const char*)) {
  inst->listener->onSetInfo = onSetInfo;
}

extern "C" void wkconverter_on_error(wkconverter_t inst,
                                     void (*onError)(void*, const char*)) {
  inst->listener->onError = onError;
}

extern "C" void wkconverter_on_progress(wkconverter_t inst,
                                        void (*onProgress)(void*, int)) {
